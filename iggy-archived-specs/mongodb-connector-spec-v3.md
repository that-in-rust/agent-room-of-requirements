...
This section lists every fix applied to v2. Each fix is tagged with a `<!-- v3-fix: NEW-N -->` comment at the point of change in the spec.

| ID | Severity | Description |
|----|----------|-------------|
| NEW-1 | Medium | Missing integration test fixture implementations — v2 has test checklist but no MongoDBContainer, MongoDBSinkFixture, MongoDBSourceFixture modeled on the real TestFixture trait |
| NEW-2 | Medium | Missing env var override documentation — the runtime supports IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_* pattern for overriding any plugin_config field. Must document this. |
| NEW-3 | Medium | Missing exact workspace Cargo.toml changes — must add both crates to workspace.members AND add mongodb to workspace.dependencies |
| NEW-4 | Minor | ProducedMessage.payload is Vec<u8> (raw bytes), NOT Payload enum — v2 correctly uses simd_json::to_vec() to serialize to bytes, but doesn't call this out as a critical difference from ConsumedMessage.payload which IS a Payload enum |
| NEW-5 | Minor | State file persistence path — runtime stores source state at `source_<key>.state` file. This means the key in config.toml determines the state filename. Document this. |
| NEW-6 | Minor | Plugin path resolution — runtime searches multiple directories for the .so/.dylib file. The path in config.toml can be relative. Document the resolution logic and naming convention (libiggy_connector_mongodb_sink.so / .dylib). |
| NEW-7 | Minor | Config format conversion — runtime converts TOML plugin_config to JSON string before FFI. The plugin receives JSON, never TOML. This is why the config struct uses `serde::Deserialize` (from JSON), not a TOML parser. |
| NEW-8 | Minor | Transform pipeline — runtime can apply transforms between message reception and delivery. The connector doesn't need to handle transforms — they're applied by the runtime. But the connector's Cargo.toml needs to list the transform-related deps that cargo-machete might flag. |
...
2.5. Complete Dependency Graph (NEW in v3)
...
15. Integration Test Fixtures (NEW in v3)
16. Exact Workspace Changes (NEW in v3)
17. Environment Variable Override Reference (NEW in v3)
...
## 2.5. Complete Dependency Graph

This section provides ASCII diagrams of the full dependency graph: the existing connector
ecosystem, the FFI data flow, and the exact interfaces the MongoDB connectors touch.

### Diagram 1: Existing Connector Ecosystem (what we're adding to)
...
```
                    ...
                    |                                                                  |
                    |  main.rs -----------+---- source.rs ---- source::init()           |
                    |                    |                    source::handle()          |
                    |                    +---- sink.rs ------- sink::init()             |
                    |                                        sink::consume()           |
                    |                                                                  |
                    |  +--- dlopen2 -------------------------------------------+       |
                    |  |  Container<SinkApi>     Container<SourceApi>          |       |
                    |  |  +- open()              +- open()                     |       |
                    |  |  +- consume()           +- handle()                   |       |
                    |  |  +- close()             +- close()                    |       |
                    |  |  +- version()           +- version()                  |       |
                    |  +-------------------------------------------------------+       |
                    +------------------+----------------------------+-----------------+
                                       | dlopen2::load()            | dlopen2::load()
                    +------------------v---------+     +------------v---------------+
                    |   CONNECTOR SDK             |     |   CONNECTOR SDK            |
                    |  (core/connectors/sdk/)     |     |  (core/connectors/sdk/)    |
                    |                             |     |                            |
                    |  sink_connector! macro      |     |  source_connector! macro   |
                    |  +- extern "C" open()       |     |  +- extern "C" open()      |
                    |  +- extern "C" consume()    |     |  +- extern "C" handle()    |
                    |  +- extern "C" close()      |     |  +- extern "C" close()     |
                    |  +- extern "C" version()    |     |  +- extern "C" version()   |
                    |                             |     |                            |
                    |  SinkContainer<T>           |     |  SourceContainer<T>        |
                    |  +- open(factory)           |     |  +- open(factory)          |
                    |  +- consume(ptrs)           |     |  +- handle(callback)       |
                    |  +- close()                 |     |  +- close()                |
                    |                             |     |                            |
                    |  trait Sink                 |     |  trait Source              |
                    |  +- open(&mut self)         |     |  +- open(&mut self)        |
                    |  +- consume(&self, ...)     |     |  +- poll(&self) -> Produced|
                    |  +- close(&mut self)        |     |  +- close(&mut self)       |
                    +---------+-------------------+     +------------+---------------+
                              | impl Sink for X                     | impl Source for X
          +-------------------+-------------------+                 |
          |                   |                   |                 |
    +-----v------+  +---------v------+  +---------v-----+  +-------v----------+
    | StdoutSink |  | PostgresSink   |  |ElasticSearch  |  | PostgresSource   |
    | (105 LOC)  |  | (799 LOC)      |  |Sink (383 LOC) |  | (1410 LOC)       |
    +------------+  +----------------+  +---------------+  +------------------+
    +------------+  +----------------+                     +------------------+
    |QuickwitSink|  | IcebergSink    |                     |ElasticSearch     |
    | (210 LOC)  |  | (multi-file)   |                     |Source (965 LOC)  |
    +------------+  +----------------+                     +------------------+
                                                           +------------------+
                                                           | RandomSource     |
    +- - - - - -+  +- - - - - - - -+                      | (example)        |
    | MongoDBSink|  | MongoDBSource |  <-- NEW             +------------------+
    |  (NEW)     |  |   (NEW)       |
    +- - - - - -+  +- - - - - - - -+
```

### Diagram 2: Serialization Boundaries (data flow through FFI)
...
```
    ...
    |  Runtime (Rust)                                                        |
    |  +-------------+                                                       |
    |  | Config TOML  |--[TOML parse]--> SinkConfig.plugin_config            |
    |  | config.toml  |                  (serde_json::Value)                  |
    |  +-------------+                       |                               |
    |                          [serde_json::to_string()]                      |
    |                                        v                               |
    |  FFI boundary ==== config_ptr: *const u8 (UTF-8 JSON) =========        |
    |                                        |                               |
    |  Plugin (cdylib)       [serde_json::from_str::<Config>()]              |
    |                                        v                               |
    |                              MongoDBSinkConfig struct                   |
    |                                                                        |
    |  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -   |
    |                                                                        |
    |  Runtime (Rust)                                                        |
    |  +------------------+                                                  |
    |  | IggyConsumer     |--> Raw Iggy messages                             |
    |  | (poll messages)  |          |                                        |
    |  +------------------+          |                                        |
    |              [postcard::to_allocvec(&TopicMetadata)]                    |
    |              [postcard::to_allocvec(&MessagesMetadata)]                 |
    |              [postcard::to_allocvec(&RawMessages)]                      |
    |                                v                                       |
    |  FFI boundary === topic_meta_ptr + messages_meta_ptr + messages_ptr ===|
    |                                |                                       |
    |  Plugin (cdylib)  [postcard::from_bytes::<TopicMetadata>()]             |
    |                   [postcard::from_bytes::<MessagesMetadata>()]          |
    |                   [postcard::from_bytes::<RawMessages>()]               |
    |                   [Schema::try_into_payload() per message]              |
    |                                v                                       |
    |                   Vec<ConsumedMessage> passed to Sink::consume()        |
    |                                |                                       |
    |                   MongoDBSink::consume()                                |
    |                                |                                       |
    |                   [mongodb driver] insert_many() --> MongoDB            |
    +------------------------------------------------------------------------+
    ...
    |  Runtime (Rust)                                                        |
    |  +-------------+   +-----------------+                                 |
    |  | Config TOML  |   | State file      |                                 |
    |  | config.toml  |   | source_mongodb  |                                 |
    |  |              |   |   .state        |                                 |
    |  +------+------+   +-------+---------+                                 |
    |         |  [JSON string]   |  [raw bytes from file]                     |
    |         v                  v                                            |
    |  FFI === config_ptr === state_ptr =====================================  |
    |         |                  |                                            |
    |  Plugin | [serde_json]     | [ConnectorState(bytes)]                    |
    |         v                  v                                            |
    |  MongoDBSourceConfig    rmp_serde::from_slice() -> SourceState          |
    |                          (MessagePack deserialization)                  |
    |                                                                        |
    |  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -   |
    |                                                                        |
    |  Source::poll() called in async loop by handle_messages()              |
    |         |                                                              |
    |  MongoDBSource::poll()                                                 |
    |         |                                                              |
    |  [mongodb driver] find()/watch() <-- MongoDB                           |
    |         |                                                              |
    |         v                                                              |
    |  ProducedMessages {                                                    |
    |    schema: Schema::Json,                                               |
    |    messages: Vec<ProducedMessage>,                                      |
    |    state: Some(ConnectorState(rmp_serde::to_vec(&SourceState)))         |
    |  }                                                                     |
    |         |                                                              |
    |  [postcard::to_allocvec(&produced_messages)]                           |
    |         |                                                              |
    |  FFI === callback(plugin_id, messages_ptr, messages_len) ===========   |
    |         |                                                              |
    |  Runtime: deserialize, apply transforms, send to IggyProducer          |
    |         |                                                              |
    |  [Save state file: source_mongodb.state]                               |
    +------------------------------------------------------------------------+
```

### Diagram 3: Interfaces MongoDBSink Touches (with method signatures)

```
    +-----------------------------------------------------------------------+
    |                    INTERFACES MONGODB SINK TOUCHES                     |
    |                                                                       |
    |  IMPLEMENTS                                                           |
    |  +------------------------------------------+                        |
    |  | trait Sink (sdk/src/lib.rs:114-129)       |                        |
    |  |                                           |                        |
    ...
    |  |   messages_metadata: MessagesMetadata,    |  <-- by value, not ref |
    |  |   messages: Vec<ConsumedMessage>,          |  <-- owned Vec         |
    |  | ) -> Result<(), Error>                    |                        |
    |  |                                           |                        |
    ...
    |  +------------------------------------------+                        |
    |                                                                       |
    |  USES (SDK types consumed)                                            |
    |  +------------------------------------------+                        |
    |  | TopicMetadata { stream, topic }           | #[repr(C)]            |
    |  | MessagesMetadata { partition_id,          | #[repr(C)]            |
    |  |   current_offset, schema }                |                        |
    |  | ConsumedMessage { id: u128, offset: u64,  | #[repr(C)]            |
    |  |   checksum: u64, timestamp: u64,          |                        |
    |  |   origin_timestamp: u64,                  |                        |
    |  |   headers: Option<HashMap<...>>,           |                        |
    |  |   payload: Payload }                      |                        |
    |  | Payload::Json(simd_json::OwnedValue)      |                        |
    |  | Payload::Raw(Vec<u8>)                     |                        |
    |  | Payload::Text(String)                     |                        |
    |  | Error { InitError, CannotStoreData,        |                        |
    |  |   InvalidJsonPayload, InvalidTextPayload,  |                        |
    |  |   InvalidRecord, ... }                    |                        |
    |  +------------------------------------------+                        |
    |                                                                       |
    |  INVOKES (macro)                                                      |
    ...
    |  |   generates:                              |                        |
    |  |   static INSTANCES: DashMap<u32,          |                        |
    |  |     SinkContainer<MongoDBSink>>           |                        |
    |  |   extern "C" fn open(...)                 |                        |
    |  |   extern "C" fn consume(...)              |                        |
    |  |   extern "C" fn close(...)                |                        |
    |  |   extern "C" fn version()                 |                        |
    |  |                                           |                        |
    |  | Constructor called by macro:              |                        |
    |  |   MongoDBSink::new(id: u32,               |                        |
    |  |     config: MongoDBSinkConfig) -> Self    | <-- EXACT signature    |
    |  +------------------------------------------+                        |
    |                                                                       |
    |  CALLS (external)                                                     |
    |  +------------------------------------------+                        |
    ...
    |  | Collection<Document>::insert_many()       | <-- fluent API v3      |
    |  |   .ordered(false).await                   |                        |
...
### Diagram 4: Interfaces MongoDBSource Touches

```
    +-----------------------------------------------------------------------+
    |                  INTERFACES MONGODB SOURCE TOUCHES                    |
    |                                                                       |
    |  IMPLEMENTS                                                           |
    |  +------------------------------------------+                        |
    |  | trait Source (sdk/src/lib.rs:100-110)     |                        |
    |  |                                           |                        |
    |  | async fn open(&mut self)                  |                        |
    |  |   -> Result<(), Error>                    |                        |
    |  |                                           |                        |
    |  | async fn poll(&self)                      | <-- &self (Arc-wrapped)|
    ...
    |  +------------------------------------------+                        |
    |                                                                       |
    |  USES (SDK types produced)                                            |
    |  +------------------------------------------+                        |
    |  | ProducedMessages { schema, messages,      | #[repr(C)]            |
    |  |   state: Option<ConnectorState> }         |                        |
    |  | ProducedMessage { id: Option<u128>,        | #[repr(C)]            |
    |  |   checksum: Option<u64>,                  |                        |
    |  |   timestamp: Option<u64>,                 |                        |
    |  |   origin_timestamp: Option<u64>,           |                        |
    |  |   headers: Option<HashMap<...>>,           |                        |
    |  |   payload: Vec<u8> }                      | <-- raw bytes, NOT Payload
    |  | ConnectorState(Vec<u8>)                   |                        |
    |  |   .deserialize::<T>() -> rmp_serde        |                        |
    |  |   ::serialize(&T) -> rmp_serde            |                        |
    |  | Schema::Json (enum variant)               |                        |
    |  | Error { InitError, InvalidRecord, ... }   |                        |
    |  +------------------------------------------+                        |
    |                                                                       |
    |  INVOKES (macro)                                                      |
    |  +------------------------------------------+                        |
    ...
    |  |   generates:                              |                        |
    |  |   static INSTANCES: DashMap<u32,          |                        |
    |  |     SourceContainer<MongoDBSource>>        |                        |
    |  |   extern "C" fn open(...)                 | <-- has state_ptr/len  |
    |  |   extern "C" fn handle(...)               | <-- NOT consume!       |
    |  |   extern "C" fn close(...)                |                        |
    |  |   extern "C" fn version()                 |                        |
    |  |                                           |                        |
    |  | Constructor called by macro:              |                        |
    |  |   MongoDBSource::new(id: u32,             |                        |
    |  |     config: MongoDBSourceConfig,           |                        |
    |  |     state: Option<ConnectorState>          | <-- 3 params, not 2!  |
    |  |   ) -> Self                               |                        |
    |  +------------------------------------------+                        |
    |                                                                       |
    |  CRITICAL: poll() takes &self because SourceContainer wraps in Arc<T> |
    |  This means all mutable state MUST use interior mutability:           |
    |  +- Mutex<SourceState> for offset tracking                            |
    |  +- Mutex<Option<ChangeStream<Document>>> for persistent cursor       |
    |                                                                       |
    |  CALLS (external)                                                     |
    |  +------------------------------------------+                        |
    |  | mongodb::Client::with_options()           |                        |
    |  | Collection<Document>::find()              | <-- fluent API v3      |
    |  |   .sort(...).limit(n).await               |                        |
    |  | Database::watch()                         | <-- change stream      |
    |  |   .pipeline(...).batch_size(n)            |                        |
    |  |   .full_document(...).resume_after(...)   |                        |
    |  |   .await                                  |                        |
    |  | simd_json::to_vec(&record)                |                        |
    |  | Uuid::new_v4().as_u128()                  |                        |
    |  +------------------------------------------+                        |
    +-----------------------------------------------------------------------+
```

### Diagram 5: Workspace Changes Required
...
```
    ...
    |  members = [                                    |
    |    ...                                          |
    |+   "core/connectors/sinks/mongodb_sink",        | <-- ADD
    |+   "core/connectors/sources/mongodb_source",    | <-- ADD
    |    ...                                          |
    |  ]                                              |
    |                                                 |
    |  [workspace.dependencies]                       |
    |    ...                                          |
    |+   mongodb = { version = "3",                   | <-- ADD
    ...
    |  +-- fixtures/mongodb/                          | <-- NEW
    |      +-- mod.rs                                 |
    |      +-- container.rs (MongoDBContainer)         |
    |      +-- sink.rs (MongoDBSinkFixture)            |
    |      +-- source.rs (MongoDBSourceFixture)        |
    +------------------------------------------------+
```
...
pub struct ConsumedMessage {
    ...
    pub payload: Payload,   // <-- Payload enum (Json/Raw/Text/Proto/FlatBuffer)
}
...
// <!-- v3-fix: NEW-4 -->
// CRITICAL DIFFERENCE: ProducedMessage.payload is Vec<u8> (raw bytes), NOT the Payload enum.
// ConsumedMessage.payload IS a Payload enum (Json/Raw/Text/etc.).
// When building ProducedMessages in MongoDBSource::poll(), you must serialize your record
// to bytes first (e.g. simd_json::to_vec(&record)) and store those bytes as payload: Vec<u8>.
// The runtime will wrap these bytes in the appropriate Payload variant based on the Schema.
#[repr(C)]
#[derive(Debug, Serialize, Deserialize)]
pub struct ProducedMessage {
    ...
    pub headers: Option<HashMap<HeaderKey, HeaderValue>>,
    pub payload: Vec<u8>,   // <-- raw bytes, NOT Payload enum
}
...
### Config format conversion from TOML to JSON

<!-- v3-fix: NEW-7 -->

**Important**: The runtime reads `config.toml`, parses the `[plugin_config]` section as TOML, then
converts it to a JSON string before passing it to the plugin via the FFI `config_ptr`. The plugin
**never sees TOML** — it only ever receives a UTF-8 JSON string.

This is why the `MongoDBSinkConfig` and `MongoDBSourceConfig` structs derive `serde::Deserialize`
(deserialized from JSON via `serde_json::from_str`), not `toml::de::Deserializer`.

```
config.toml [plugin_config] section
    |
    v TOML parse by runtime
    |
    v serde_json::Value in memory
    |
    v serde_json::to_string() -> JSON UTF-8 string
    |
    v FFI config_ptr: *const u8
    |
    v serde_json::from_str::<MongoDBSinkConfig>() in plugin
    |
    v MongoDBSinkConfig struct
```
...
The `path` field in `config.toml` names the compiled shared library **without** the platform
extension. The runtime appends the correct extension:

| Platform | Extension | Example filename |
|----------|-----------|-----------------|
| Linux | `.so` | `libiggy_connector_mongodb_sink.so` |
| macOS | `.dylib` | `libiggy_connector_mongodb_sink.dylib` |
| Windows | `.dll` | `iggy_connector_mongodb_sink.dll` |

The path value `"../../target/release/libiggy_connector_mongodb_sink"` is relative to the
`config.toml` file location. The runtime resolves it relative to the config file's parent
directory, then appends the platform extension.

The Rust `crate-type = ["cdylib", "lib"]` in Cargo.toml produces:
- `libiggy_connector_mongodb_sink.so` (Linux)
- `libiggy_connector_mongodb_sink.dylib` (macOS)
- `iggy_connector_mongodb_sink.dll` (Windows)
...
pub struct MongoDBSourceConfig {
    ...
    /// Field to set to true after reading in polling mode.
...
struct SourceState {
    ...
    /// Serialized as a JSON string of the BSON resume token document.
    ...
    async fn poll_collections(&self) -> Result<Vec<ProducedMessage>, Error> {
        ...
        for collection_name in &self.config.collections {
            ...
            while let Some(result) = cursor.next().await {
                ...
                // <!-- v3-fix: NEW-4 — payload is Vec<u8> (raw bytes), serialize with simd_json -->
                ...
                messages.push(ProducedMessage {
                    ...
                    payload,  // Vec<u8>, not Payload enum
                });
                total_processed += 1;
            }
...
impl MongoDBSource {
            ...
            match serde_json::from_str::<serde_json::Value>(token_json) {
                Ok(json_val) => {
                    match mongodb::bson::to_document(&json_val) {
                        ...
                        Err(e) => {
                            warn!("Failed to parse saved resume token, starting fresh: {e}");
                        }
                    }
                }
                Err(e) => {
    ...
    fn change_event_to_message(
        ...
        let operation_type = match &event.operation_type {
            ...
            OperationType::Invalidate => return Ok(None),
        ...
        // <!-- v3-fix: NEW-4 — payload is Vec<u8> raw bytes, NOT a Payload enum -->
        let payload = simd_json::to_vec(&record)
            .map_err(|_| Error::InvalidRecord)?;
...
**Note on the `mongodb` dependency**: Both crates reference `mongodb` from the workspace. The
workspace root Cargo.toml must declare `mongodb = { version = "3", features = ["tokio-runtime"] }`
in `[workspace.dependencies]`. See Section 16 for the exact changes.
...
The runtime persists the `ConnectorState` bytes to a file on disk. The filename is derived from
the `key` field in `config.toml`:

```
source_<key>.state
```

For the MongoDB source with `key = "mongodb"`, the state file is `source_mongodb.state`. This
file is written in the runtime's working directory. If you need to reset state (e.g., to re-read
from the beginning), delete this file before restarting the connector runtime.

The sink does not have a corresponding state file because sinks are stateless from the runtime's
perspective.
...
- On restart, it is deserialized back to a BSON Document and passed via `.resume_after()` on
  the `watch()` fluent builder (v2 fix for BUG-3).
...
The root workspace Cargo.toml must add both crates. See Section 16 for exact changes.
...
- [ ] Write `Cargo.toml` (section 8.1) — reference `mongodb` from workspace; include `prost` in cargo-machete ignored list
...
- [ ] Add to workspace Cargo.toml (section 16)

### Phase 2: MongoDBSource

- [ ] Create `core/connectors/sources/mongodb_source/` directory
- [ ] Write `Cargo.toml` (section 8.2) — reference `mongodb` from workspace; include `prost` in cargo-machete ignored list
- [ ] Write `src/lib.rs` with Apache 2.0 license header:
  ...
  - [ ] `change_event_to_message()` — include_metadata controls envelope fields; payload is Vec<u8> (section 6.7)
...
- [ ] Add `mongodb = { version = "3", features = ["tokio-runtime"] }` to workspace `[workspace.dependencies]`
...
### Phase 4: Integration test fixtures (NEW in v3)

- [ ] Create `core/integration/tests/connectors/fixtures/mongodb/mod.rs`
- [ ] Implement `MongoDBContainer` (section 15)
- [ ] Implement `MongoDBSinkFixture` (section 15)
- [ ] Implement `MongoDBSourceFixture` (section 15)
- [ ] Run integration tests via `cargo test -p integration_tests`
...
The connector runtime integration tests use a `TestFixture` trait to abstract over the test
infrastructure (container startup, env vars, seed data). MongoDB fixtures must implement this
trait, modeled on how `PostgresContainer`, `PostgresSinkFixture`, and `PostgresSourceFixture`
are implemented.

### TestFixture trait (from real source)

```rust
// core/integration/tests/connectors/fixtures/mod.rs (real)
#[async_trait]
pub trait TestFixture: Sized + Send {
    async fn setup() -> Result<Self, TestBinaryError>;
    fn connectors_runtime_envs(&self) -> HashMap<String, String>;
    async fn seed(&self, _client: &IggyClient) -> Result<(), SeedError> { Ok(()) }
    fn has_seed(&self) -> bool { false }
}
```

### 15.1 MongoDBContainer

Modeled on `PostgresContainer`. Uses `testcontainers` crate to start a MongoDB instance.
...
impl MongoDBContainer {
    pub async fn start() -> Result<Self, Box<dyn std::error::Error>> {
        // Use the official MongoDB image. For change stream support (replica set),
        // use mongodb/mongodb-community-server with --replSet rs0 command.
        // For polling-only tests, a standalone is sufficient.
        ...
        let connection_uri = format!("mongodb://{}:{}", host, port);
    ...
    /// Start with replica set support (required for change stream tests).
    /// Uses the community server image which supports --replSet initialization.
    pub async fn start_with_replica_set() -> Result<Self, Box<dyn std::error::Error>> {
        let image = GenericImage::new("mongo", "7.0")
            .with_wait_for(WaitFor::message_on_stdout("Waiting for connections"))
            .with_exposed_port(27017.tcp())
            .with_cmd(vec!["--replSet", "rs0", "--bind_ip_all"]);
        ...
        let client = mongodb::Client::with_uri_str(&connection_uri).await?;
        client
            .database("admin")
            .run_command(mongodb::bson::doc! {
                "replSetInitiate": {
                    "_id": "rs0",
                    "members": [{ "_id": 0, "host": format!("{}:{}", host, port) }]
                }
            })
            .await?;
...
### 15.2 MongoDBSinkFixture

Modeled on `PostgresSinkFixture`. Implements `TestFixture` and provides the env vars that
override the plugin_config connection URI.

```rust
// core/integration/tests/connectors/fixtures/mongodb/sink.rs

use super::container::MongoDBContainer;
use crate::fixtures::TestFixture;
use async_trait::async_trait;
use std::collections::HashMap;

pub struct MongoDBSinkFixture {
    pub container: MongoDBContainer,
    pub database: String,
...
impl TestFixture for MongoDBSinkFixture {
    async fn setup() -> Result<Self, crate::fixtures::TestBinaryError> {
        let container = MongoDBContainer::start()
            .await
            .map_err(|e| crate::fixtures::TestBinaryError::Setup(e.to_string()))?;

        Ok(MongoDBSinkFixture {
            container,
            database: "iggy_test".to_string(),
            collection: "test_messages".to_string(),
        })
    }

    fn connectors_runtime_envs(&self) -> HashMap<String, String> {
        // <!-- v3-fix: NEW-2 — env vars follow IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_* pattern -->
        let mut envs = HashMap::new();
        envs.insert(
            "IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_CONNECTION_URI".to_string(),
            self.container.connection_uri.clone(),
        );
        envs.insert(
            "IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_DATABASE".to_string(),
            self.database.clone(),
        );
        envs.insert(
            "IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_COLLECTION".to_string(),
...
impl MongoDBSinkFixture {
    /// Get a mongodb::Collection handle to verify documents were inserted.
    pub async fn get_collection(
        &self,
    ) -> Result<mongodb::Collection<mongodb::bson::Document>, Box<dyn std::error::Error>> {
        let client = mongodb::Client::with_uri_str(&self.container.connection_uri).await?;
...
### 15.3 MongoDBSourceFixture

Modeled on `PostgresSourceFixture`. Seeds MongoDB with test documents and provides env vars
so the runtime uses the test container.
...
impl TestFixture for MongoDBSourceFixture {
    async fn setup() -> Result<Self, crate::fixtures::TestBinaryError> {
        ...
        Ok(MongoDBSourceFixture {
            container,
            database: "iggy_source_test".to_string(),
    ...
    fn connectors_runtime_envs(&self) -> HashMap<String, String> {
        // <!-- v3-fix: NEW-2 — env vars follow IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_* pattern -->
        let mut envs = HashMap::new();
        envs.insert(
            "IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_CONNECTION_URI".to_string(),
            self.container.connection_uri.clone(),
        );
        envs.insert(
            "IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_DATABASE".to_string(),
            self.database.clone(),
        );
        envs.insert(
            "IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_MODE".to_string(),
            self.mode.clone(),
        );
        envs.insert(
            "IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_COLLECTIONS".to_string(),
            // Collections is a Vec<String>; the runtime's env override format for arrays
            // uses comma-separated values or JSON array string depending on runtime version.
            // Use JSON array format for safety:
    ...
    async fn seed(
        &self,
        _client: &iggy::clients::client::IggyClient,
    ) -> Result<(), crate::fixtures::SeedError> {
        // Insert test documents directly into MongoDB so the source can read them
        let client = mongodb::Client::with_uri_str(&self.container.connection_uri)
            .await
            .map_err(|e| crate::fixtures::SeedError::Seed(e.to_string()))?;

        let collection = client
            .database(&self.database)
            .collection::<mongodb::bson::Document>(&self.collection);

        // Seed 10 test documents
        let docs: Vec<mongodb::bson::Document> = (1..=10)
            .map(|i| doc! {
                "event_id": i as i32,
...
pub use container::MongoDBContainer;
pub use sink::MongoDBSinkFixture;
pub use source::MongoDBSourceFixture;
```

---

## 16. Exact Workspace Changes

<!-- v3-fix: NEW-3 -->

The root workspace `Cargo.toml` (at the repository root, not `core/connectors/Cargo.toml`)
requires these exact additions:

### 16.1 Add to `[workspace] members`

Find the `[workspace]` section's `members` array and add the two new crates. The exact path
depends on whether the workspace root is the repo root or `core/connectors/`. Based on how
existing connectors are referenced, add:

```toml
# In [workspace] members array — add these two lines:
"core/connectors/sinks/mongodb_sink",
"core/connectors/sources/mongodb_source",
```

### 16.2 Add to `[workspace.dependencies]`

```toml
# In [workspace.dependencies] — add this entry:
mongodb = { version = "3", features = ["tokio-runtime"] }
```

The `tokio-runtime` feature enables the async client backed by Tokio. The `sync` feature (for
the blocking client) is NOT needed.
...
### 16.4 Why workspace.dependencies, not inline version

Both `mongodb_sink/Cargo.toml` and `mongodb_source/Cargo.toml` reference:
```toml
mongodb = { workspace = true }
```

This pattern ensures:
1. A single version is used across both crates (avoiding duplicate compilation)
2. The version is updated in one place
3. It follows the pattern all other shared deps use (`tokio`, `serde`, etc.)

---

## 17. Environment Variable Override Reference

<!-- v3-fix: NEW-2 -->

The Iggy connector runtime supports overriding any `[plugin_config]` field via environment
variables. This is useful for injecting secrets (connection URIs with passwords), CI test
configuration, and Kubernetes secrets without modifying `config.toml`.

### 17.1 Override pattern

The environment variable format is:

```
IGGY_CONNECTORS_<SINK|SOURCE>_<KEY>_PLUGIN_CONFIG_<FIELD>
```

Where:
- `<SINK|SOURCE>` is `SINK` or `SOURCE` (uppercase)
- `<KEY>` is the `key` field from `config.toml`, uppercased (e.g., `MONGODB`)
- `<FIELD>` is the config field name, uppercased with underscores

For `key = "mongodb"` and a sink:
```
...
For `key = "mongodb"` and a source:
```
IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_CONNECTION_URI
IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_DATABASE
IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_MODE
IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_COLLECTIONS
```

### 17.2 MongoDBSink env var reference

| Field | Environment Variable |
|-------|----------------------|
| `connection_uri` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_CONNECTION_URI` |
| `database` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_DATABASE` |
| `collection` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_COLLECTION` |
| `batch_size` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_BATCH_SIZE` |
| `max_pool_size` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_MAX_POOL_SIZE` |
| `include_metadata` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_INCLUDE_METADATA` |
| `include_checksum` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_INCLUDE_CHECKSUM` |
| `include_origin_timestamp` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_INCLUDE_ORIGIN_TIMESTAMP` |
| `payload_format` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_PAYLOAD_FORMAT` |
| `ordered_inserts` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_ORDERED_INSERTS` |
| `verbose_logging` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_VERBOSE_LOGGING` |
| `max_retries` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_MAX_RETRIES` |
| `retry_delay` | `IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_RETRY_DELAY` |

### 17.3 MongoDBSource env var reference

| Field | Environment Variable |
|-------|----------------------|
| `connection_uri` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_CONNECTION_URI` |
| `database` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_DATABASE` |
| `mode` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_MODE` |
| `collections` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_COLLECTIONS` |
| `filter_collections` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_FILTER_COLLECTIONS` |
| `poll_interval` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_POLL_INTERVAL` |
| `batch_size` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_BATCH_SIZE` |
| `tracking_field` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_TRACKING_FIELD` |
| `initial_offset` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_INITIAL_OFFSET` |
| `max_pool_size` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_MAX_POOL_SIZE` |
| `resume_token` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_RESUME_TOKEN` |
| `capture_operations` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_CAPTURE_OPERATIONS` |
| `full_document_on_update` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_FULL_DOCUMENT_ON_UPDATE` |
| `include_metadata` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_INCLUDE_METADATA` |
| `delete_after_read` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_DELETE_AFTER_READ` |
| `processed_field` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_PROCESSED_FIELD` |
| `verbose_logging` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_VERBOSE_LOGGING` |
| `max_retries` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_MAX_RETRIES` |
| `retry_delay` | `IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_RETRY_DELAY` |

### 17.4 Usage examples

**Override connection URI for CI**:
```bash
export IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_CONNECTION_URI="mongodb://ci-host:27017"
./iggy-connectors --config config.toml
```

**Kubernetes secret injection**:
```yaml
env:
  - name: IGGY_CONNECTORS_SINK_MONGODB_PLUGIN_CONFIG_CONNECTION_URI
    valueFrom:
      secretKeyRef:
...
**Override multiple fields in a test**:
```bash
IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_CONNECTION_URI="mongodb://localhost:27017" \
IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_DATABASE="test_db" \
IGGY_CONNECTORS_SOURCE_MONGODB_PLUGIN_CONFIG_MODE="polling" \
...
efficiency. The spec defaults to String for simplicity.
...
## Appendix D: ProducedMessage vs ConsumedMessage Payload Types

<!-- v3-fix: NEW-4 -->

This is a critical difference between reading from Iggy (sink) and writing to Iggy (source).

### ConsumedMessage.payload — IS the Payload enum

When MongoDBSink receives messages from Iggy via `consume()`, each message's payload is a
`Payload` enum variant:

```rust
pub enum Payload {
    Json(simd_json::OwnedValue),   // Already parsed JSON
...
The sink calls `message.payload.clone().try_into_vec()` to convert to raw bytes, then
handles those bytes according to `payload_format` config (parse as JSON, store as Binary, etc.).

### ProducedMessage.payload — is Vec<u8> raw bytes

When MongoDBSource produces messages for Iggy via `poll()`, each message's payload field is
a plain `Vec<u8>` — **not** a `Payload` enum:
...
The source must serialize its output to bytes before storing in `payload`. The spec uses
`simd_json::to_vec(&record)` to serialize `MongoChangeRecord` to JSON bytes. The runtime then
wraps these bytes in the appropriate `Payload` variant based on `Schema::Json`.

**The full serialization chain for source output:**

```
MongoChangeRecord struct
    |
    v simd_json::to_vec(&record) -> Vec<u8>
    |
ProducedMessage { payload: Vec<u8> }
    |
    v postcard::to_allocvec(&produced_messages) [by runtime FFI layer]
    |
    v [runtime deserializes, wraps in Payload::Json based on schema]
    |
IggyProducer sends message to Iggy stream
```

---

*Specification written by the Iggy connector design process.*
*Version 3.0 incorporates codebase dependency graph analysis, integration test fixture patterns,*
*workspace configuration details, and env var override documentation.*
