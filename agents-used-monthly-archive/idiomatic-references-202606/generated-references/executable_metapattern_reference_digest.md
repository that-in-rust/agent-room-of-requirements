...
| --- | --- | --- | --- |
| agents-used-monthly-archive/codex-skills-202602/executable-specs-01/references/meta-patterns-reference.md | local_corpus_source_material | local_corpus_sourced_fact | reference detail file for deeper pattern evidence |
| agents-used-monthly-archive/codex-skills-202603/executable-specs-01/references/meta-patterns-reference.md | local_corpus_source_material | local_corpus_sourced_fact | reference detail file for deeper pattern evidence |
| unclassified-yet/Executable Specifications Meta Patterns Reference.md | local_corpus_source_material | local_corpus_sourced_fact | local agent-usable source material |
...
| --- | ---: | --- | --- |
| `executable_metapattern_reference_digest` | 95 | default_adoption_pattern_tier | Source Map First: Load local executable metapattern material before synthesizing reference digest recommendations. |
| `executable_metapattern_reference_digest` | 91 | default_adoption_pattern_tier | Evidence Boundary Split: Separate local facts, external facts, and inference before giving agent instructions. |
| `executable_metapattern_reference_digest` | 88 | default_adoption_pattern_tier | Verification Gate Coupling: Attach each recommendation to at least one command, checklist, or review gate. |

## Idiomatic Thesis Synthesis Statement

`local_corpus_sourced_fact`: The local row for `executable_metapattern_reference_digest` contains 3 source path(s), which should be treated as the first retrieval surface for this theme.
`external_research_sourced_fact`: The external source map provides public documentation, repository, or ecosystem analogues where available.
`combined_evidence_inference_note`: Reliable use of Executable Metapattern Reference Digest comes from loading the local corpus first, checking public ecosystem guidance second, and converting both into verification-backed agent decisions.

## Local Corpus Source Map

| local_source_filepath_value | local_source_title_text | local_source_heading_signals | local_source_usage_role |
| --- | --- | --- | --- |
| agents-used-monthly-archive/codex-skills-202602/executable-specs-01/references/meta-patterns-reference.md | AI-Native Meta-Patterns Digest | AI-Native Meta-Patterns Digest; 1) Measured Outcomes to Preserve; 2) Seven Core Principles (Operational Form); 3) Four-Word Naming Convention (4WNC); 4) TDD-First Loop; 5) Executable Requirement Pattern | reference detail file for deeper pattern evidence |
| agents-used-monthly-archive/codex-skills-202603/executable-specs-01/references/meta-patterns-reference.md | AI-Native Meta-Patterns Digest | AI-Native Meta-Patterns Digest; 1) Measured Outcomes to Preserve; 2) Seven Core Principles (Operational Form); 3) Four-Word Naming Convention (4WNC); 4) TDD-First Loop; 5) Executable Requirement Pattern | reference detail file for deeper pattern evidence |
| unclassified-yet/Executable Specifications Meta Patterns Reference.md | AI-Native Meta-Patterns Digest | AI-Native Meta-Patterns Digest; 1) Measured Outcomes to Preserve; 2) Seven Core Principles (Operational Form); 3) Four-Word Naming Convention (4WNC); 4) TDD-First Loop; 5) Executable Requirement Pattern | local agent-usable source material |
...
Primary user starting state: The user has a `executable_metapattern_reference_digest` task, one or more local source paths, and uncertainty about which pattern should drive implementation.
Decision being made: choosing the requirement IDs, negative cases, verification gates, and handoff evidence.
Reference opening trigger: Open this file when the task mentions executable metapattern reference digest, any mapped local source path, or an adjacent workflow with the same failure mode.

## Decision Tradeoff Guide

| decision_option_name | when_to_choose_condition | tradeoff_cost_description | verification_question_prompt |
| --- | --- | --- | --- |
| Adopt when | local corpus and external evidence agree on the executable metapattern reference digest pattern | fastest path, but can copy stale local assumptions | Does the selected pattern appear in the canonical source and current external evidence? |
| Adapt when | local sources are strong but public ecosystem guidance has changed | preserves repo fit, but requires explicit inference notes | Did the reference label the local fact, external fact, and combined inference separately? |
| Avoid when | source evidence is thin, conflicting, or unrelated to the user journey | prevents false confidence, but may require deeper research | Is there a confidence warning or adjacent reference route? |
| Cost of being wrong | wrong executable metapattern reference digest guidance can send an agent to the wrong files, tests, or abstraction | wasted implementation loop and weaker verification | Would a reviewer know what to undo and what to inspect next? |
...
| --- | --- | --- | --- |
| agents-used-monthly-archive/codex-skills-202602/executable-specs-01/references/meta-patterns-reference.md | canonical primary source | AI-Native Meta-Patterns Digest; 1) Measured Outcomes to Preserve; 2) Seven Core Principles (Operational Form) | What guidance, warning, or example should this source contribute to Executable Metapattern Reference Digest? |
| agents-used-monthly-archive/codex-skills-202603/executable-specs-01/references/meta-patterns-reference.md | supporting detail source | AI-Native Meta-Patterns Digest; 1) Measured Outcomes to Preserve; 2) Seven Core Principles (Operational Form) | What guidance, warning, or example should this source contribute to Executable Metapattern Reference Digest? |
| unclassified-yet/Executable Specifications Meta Patterns Reference.md | supporting context source | AI-Native Meta-Patterns Digest; 1) Measured Outcomes to Preserve; 2) Seven Core Principles (Operational Form) | What guidance, warning, or example should this source contribute to Executable Metapattern Reference Digest? |
...
| --- | --- | --- |
| user_goal_statement | state the user's concrete goal before applying Executable Metapattern Reference Digest | local corpus hierarchy plus critique findings |
| decision_boundary_rule | define the point where this reference should be used or avoided | decision tradeoff guide |
| verification_gate_rule | define the command, checklist, or review question that proves the artifact worked | verification gate command set |

## Worked Example Set

Good example: Use Executable Metapattern Reference Digest after loading the canonical source, confirming the external evidence boundary, and writing a verification gate before implementation.
Bad example: Use Executable Metapattern Reference Digest as a generic tutorial while ignoring the mapped local paths, source hierarchy, and cost of being wrong.
Borderline case: Use Executable Metapattern Reference Digest only after adding a confidence warning when local evidence is thin or conflicts with current ecosystem guidance.
...
- The role scenario names the user, starting state, decision, and trigger for Executable Metapattern Reference Digest.
...
Adjacent reference 1: consider the executable adjacent reference when the current task pivots away from executable metapattern reference digest.
Adjacent reference 2: consider the metapattern adjacent reference when the current task pivots away from executable metapattern reference digest.
Adjacent reference 3: consider the reference adjacent reference when the current task pivots away from executable metapattern reference digest.
...
| source_pressure_model | local heading signals include AI-Native Meta-Patterns Digest; 1) Measured Outcomes to Preserve; 2) Seven Core Principles (Operational Form); 3) Four-Word Naming Convention (4WNC);  | compare guidance against canonical local sources before following external examples |
...
Executable Metapattern Reference Digest stops being sufficient when the task spans multiple independent systems, more than one ownership boundary, unbounded source discovery, or production traffic without an explicit SLO.
...
| --- | --- |
| `official_docs_query_phrase` | executable metapattern reference digest official documentation best practices |
| `github_repository_query_phrase` | executable metapattern reference digest GitHub repository examples |
| `release_notes_query_phrase` | executable metapattern reference digest changelog release notes migration |
