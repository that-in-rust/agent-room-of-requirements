...
| --- | --- | --- | --- |
| agents-used-monthly-archive/claude-skills-202603/plugins/example-plugin/SKILL.md | local_corpus_source_material | local_corpus_sourced_fact | skill entrypoint or reusable agent prompt |
| claude-skills/plugins/example-plugin/SKILL.md | local_corpus_source_material | local_corpus_sourced_fact | skill entrypoint or reusable agent prompt |
...
| --- | ---: | --- | --- |
| `example_plugin_demonstration_patterns` | 95 | default_adoption_pattern_tier | Source Map First: Load local example plugin material before synthesizing demonstration patterns recommendations. |
| `example_plugin_demonstration_patterns` | 91 | default_adoption_pattern_tier | Evidence Boundary Split: Separate local facts, external facts, and inference before giving agent instructions. |
| `example_plugin_demonstration_patterns` | 88 | default_adoption_pattern_tier | Verification Gate Coupling: Attach each recommendation to at least one command, checklist, or review gate. |

## Idiomatic Thesis Synthesis Statement

`local_corpus_sourced_fact`: The local row for `example_plugin_demonstration_patterns` contains 2 source path(s), which should be treated as the first retrieval surface for this theme.
`external_research_sourced_fact`: The external source map provides public documentation, repository, or ecosystem analogues where available.
`combined_evidence_inference_note`: Reliable use of Example Plugin Demonstration Patterns comes from loading the local corpus first, checking public ecosystem guidance second, and converting both into verification-backed agent decisions.

## Local Corpus Source Map

| local_source_filepath_value | local_source_title_text | local_source_heading_signals | local_source_usage_role |
| --- | --- | --- | --- |
| agents-used-monthly-archive/claude-skills-202603/plugins/example-plugin/SKILL.md | example-skill | Example Skill; Overview; When This Skill Applies; Skill Structure; Required Files; Optional Supporting Files | skill entrypoint or reusable agent prompt |
| claude-skills/plugins/example-plugin/SKILL.md | example-skill | Example Skill; Overview; When This Skill Applies; Skill Structure; Required Files; Optional Supporting Files | skill entrypoint or reusable agent prompt |
...
Primary user starting state: The user has a `example_plugin_demonstration_patterns` task, one or more local source paths, and uncertainty about which pattern should drive implementation.
...
Reference opening trigger: Open this file when the task mentions example plugin demonstration patterns, any mapped local source path, or an adjacent workflow with the same failure mode.

## Decision Tradeoff Guide

| decision_option_name | when_to_choose_condition | tradeoff_cost_description | verification_question_prompt |
| --- | --- | --- | --- |
| Adopt when | local corpus and external evidence agree on the example plugin demonstration patterns pattern | fastest path, but can copy stale local assumptions | Does the selected pattern appear in the canonical source and current external evidence? |
| Adapt when | local sources are strong but public ecosystem guidance has changed | preserves repo fit, but requires explicit inference notes | Did the reference label the local fact, external fact, and combined inference separately? |
| Avoid when | source evidence is thin, conflicting, or unrelated to the user journey | prevents false confidence, but may require deeper research | Is there a confidence warning or adjacent reference route? |
| Cost of being wrong | wrong example plugin demonstration patterns guidance can send an agent to the wrong files, tests, or abstraction | wasted implementation loop and weaker verification | Would a reviewer know what to undo and what to inspect next? |
...
| --- | --- | --- | --- |
| agents-used-monthly-archive/claude-skills-202603/plugins/example-plugin/SKILL.md | canonical primary source | Example Skill; Overview; When This Skill Applies | What guidance, warning, or example should this source contribute to Example Plugin Demonstration Patterns? |
| claude-skills/plugins/example-plugin/SKILL.md | supporting context source | Example Skill; Overview; When This Skill Applies | What guidance, warning, or example should this source contribute to Example Plugin Demonstration Patterns? |

## Theme Specific Artifact

Theme specific artifact: worked example plugin demonstration patterns example with user goal, decision point, failure mode, and verification gate.

| artifact_field_name | artifact_completion_rule | evidence_source_hint |
| --- | --- | --- |
| user_goal_statement | state the user's concrete goal before applying Example Plugin Demonstration Patterns | local corpus hierarchy plus critique findings |
| decision_boundary_rule | define the point where this reference should be used or avoided | decision tradeoff guide |
| verification_gate_rule | define the command, checklist, or review question that proves the artifact worked | verification gate command set |

## Worked Example Set

Good example: Use Example Plugin Demonstration Patterns after loading the canonical source, confirming the external evidence boundary, and writing a verification gate before implementation.
Bad example: Use Example Plugin Demonstration Patterns as a generic tutorial while ignoring the mapped local paths, source hierarchy, and cost of being wrong.
Borderline case: Use Example Plugin Demonstration Patterns only after adding a confidence warning when local evidence is thin or conflicts with current ecosystem guidance.
...
- The role scenario names the user, starting state, decision, and trigger for Example Plugin Demonstration Patterns.
...
Adjacent reference 1: consider the example adjacent reference when the current task pivots away from example plugin demonstration patterns.
Adjacent reference 2: consider the plugin adjacent reference when the current task pivots away from example plugin demonstration patterns.
Adjacent reference 3: consider the demonstration adjacent reference when the current task pivots away from example plugin demonstration patterns.

## Workload Model

`combined_evidence_inference_note`: Treat Example Plugin Demonstration Patterns as a agent workflow operating reference, not as a prose summary.
...
| source_pressure_model | local heading signals include Example Skill; Overview; When This Skill Applies; Skill Structure; Required Files; Optional Supporting Files; Example Skill; Overview; When This Skill | compare guidance against canonical local sources before following external examples |
| artifact_pressure_model | required artifact is worked example plugin demonstration patterns example with user goal, decision point, failure mode, and verification gate | require the artifact before claiming the reference is operationally usable |
...
Example Plugin Demonstration Patterns stops being sufficient when the task spans multiple independent systems, more than one ownership boundary, unbounded source discovery, or production traffic without an explicit SLO.
...
| --- | --- |
| `official_docs_query_phrase` | example plugin demonstration patterns official documentation best practices |
| `github_repository_query_phrase` | example plugin demonstration patterns GitHub repository examples |
| `release_notes_query_phrase` | example plugin demonstration patterns changelog release notes migration |
