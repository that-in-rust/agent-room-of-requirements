...
`combined_evidence_inference_note`: Treat Claude Md Management Patterns as a agent workflow operating reference, not as a prose summary.
...
| source_pressure_model | local heading signals include CLAUDE.md Improver; Workflow; Phase 1: Discovery; Phase 2: Quality Assessment; Phase 3: Quality Report Output; CLAUDE.md Quality Report; CLAUDE.md Qua | compare guidance against canonical local sources before following external examples |
| artifact_pressure_model | required artifact is worked claude md management patterns example with user goal, decision point, failure mode, and verification gate | require the artifact before claiming the reference is operationally usable |
...
Claude Md Management Patterns stops being sufficient when the task spans multiple independent systems, more than one ownership boundary, unbounded source discovery, or production traffic without an explicit SLO.
