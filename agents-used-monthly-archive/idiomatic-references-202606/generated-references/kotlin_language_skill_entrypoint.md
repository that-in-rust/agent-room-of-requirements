...
`combined_evidence_inference_note`: Treat Kotlin Language Skill Entrypoint as a agent workflow operating reference, not as a prose summary.
...
| source_pressure_model | local heading signals include Kotlin Reliability; Overview; When To Use; Mode Selection; Workflow; Output Expectations | compare guidance against canonical local sources before following external examples |
...
Kotlin Language Skill Entrypoint stops being sufficient when the task spans multiple independent systems, more than one ownership boundary, unbounded source discovery, or production traffic without an explicit SLO.
