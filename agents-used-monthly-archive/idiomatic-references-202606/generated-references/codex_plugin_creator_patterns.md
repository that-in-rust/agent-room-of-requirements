...
| --- | --- | --- | --- |
| agents-used-monthly-archive/codex-skills-202603/.system/plugin-creator/SKILL.md | local_corpus_source_material | local_corpus_sourced_fact | skill entrypoint or reusable agent prompt |
| agents-used-monthly-archive/codex-skills-202603/.system/plugin-creator/references/plugin-json-spec.md | local_corpus_source_material | local_corpus_sourced_fact | reference detail file for deeper pattern evidence |
...
`local_corpus_sourced_fact`: The local row for `codex_plugin_creator_patterns` contains 2 source path(s), which should be treated as the first retrieval surface for this theme.
`external_research_sourced_fact`: The external source map provides public documentation, repository, or ecosystem analogues where available.
`combined_evidence_inference_note`: Reliable use of Codex Plugin Creator Patterns comes from loading the local corpus first, checking public ecosystem guidance second, and converting both into verification-backed agent decisions.

## Local Corpus Source Map

| local_source_filepath_value | local_source_title_text | local_source_heading_signals | local_source_usage_role |
| --- | --- | --- | --- |
| agents-used-monthly-archive/codex-skills-202603/.system/plugin-creator/SKILL.md | plugin-creator | Plugin Creator; Quick Start; Plugin names are normalized to lower-case hyphen-case and must be <= 64 chars.; The generated folder and plugin.json name are always the same.; Run from repo root (or replace .agents/... with the absolute path to this SKILL).; By default creates in <repo_root>/plugins/<plugin-name>. | skill entrypoint or reusable agent prompt |
| agents-used-monthly-archive/codex-skills-202603/.system/plugin-creator/references/plugin-json-spec.md | Plugin JSON sample spec | Plugin JSON sample spec; Field guide; Top-level fields; 'interface' fields; Path conventions and defaults; Marketplace JSON sample spec | reference detail file for deeper pattern evidence |
...
Primary user starting state: The user has a `codex_plugin_creator_patterns` task, one or more local source paths, and uncertainty about which pattern should drive implementation.
...
Reference opening trigger: Open this file when the task mentions codex plugin creator patterns, any mapped local source path, or an adjacent workflow with the same failure mode.

## Decision Tradeoff Guide

| decision_option_name | when_to_choose_condition | tradeoff_cost_description | verification_question_prompt |
| --- | --- | --- | --- |
| Adopt when | local corpus and external evidence agree on the codex plugin creator patterns pattern | fastest path, but can copy stale local assumptions | Does the selected pattern appear in the canonical source and current external evidence? |
| Adapt when | local sources are strong but public ecosystem guidance has changed | preserves repo fit, but requires explicit inference notes | Did the reference label the local fact, external fact, and combined inference separately? |
| Avoid when | source evidence is thin, conflicting, or unrelated to the user journey | prevents false confidence, but may require deeper research | Is there a confidence warning or adjacent reference route? |
| Cost of being wrong | wrong codex plugin creator patterns guidance can send an agent to the wrong files, tests, or abstraction | wasted implementation loop and weaker verification | Would a reviewer know what to undo and what to inspect next? |
...
| --- | --- | --- | --- |
| agents-used-monthly-archive/codex-skills-202603/.system/plugin-creator/SKILL.md | canonical primary source | Plugin Creator; Quick Start; Plugin names are normalized to lower-case hyphen-case and must be <= 64 chars. | What guidance, warning, or example should this source contribute to Codex Plugin Creator Patterns? |
| agents-used-monthly-archive/codex-skills-202603/.system/plugin-creator/references/plugin-json-spec.md | supporting detail source | Plugin JSON sample spec; Field guide; Top-level fields | What guidance, warning, or example should this source contribute to Codex Plugin Creator Patterns? |

## Theme Specific Artifact

Theme specific artifact: worked codex plugin creator patterns example with user goal, decision point, failure mode, and verification gate.

| artifact_field_name | artifact_completion_rule | evidence_source_hint |
| --- | --- | --- |
| user_goal_statement | state the user's concrete goal before applying Codex Plugin Creator Patterns | local corpus hierarchy plus critique findings |
| decision_boundary_rule | define the point where this reference should be used or avoided | decision tradeoff guide |
| verification_gate_rule | define the command, checklist, or review question that proves the artifact worked | verification gate command set |

## Worked Example Set

Good example: Use Codex Plugin Creator Patterns after loading the canonical source, confirming the external evidence boundary, and writing a verification gate before implementation.
Bad example: Use Codex Plugin Creator Patterns as a generic tutorial while ignoring the mapped local paths, source hierarchy, and cost of being wrong.
Borderline case: Use Codex Plugin Creator Patterns only after adding a confidence warning when local evidence is thin or conflicts with current ecosystem guidance.
...
- The role scenario names the user, starting state, decision, and trigger for Codex Plugin Creator Patterns.
...
Adjacent reference 1: consider the codex adjacent reference when the current task pivots away from codex plugin creator patterns.
Adjacent reference 2: consider the plugin adjacent reference when the current task pivots away from codex plugin creator patterns.
Adjacent reference 3: consider the creator adjacent reference when the current task pivots away from codex plugin creator patterns.

## Workload Model

`combined_evidence_inference_note`: Treat Codex Plugin Creator Patterns as a agent workflow operating reference, not as a prose summary.
...
| source_pressure_model | local heading signals include Plugin Creator; Quick Start; Plugin names are normalized to lower-case hyphen-case and must be <= 64 chars.; The generated folder and plugin.json name | compare guidance against canonical local sources before following external examples |
| artifact_pressure_model | required artifact is worked codex plugin creator patterns example with user goal, decision point, failure mode, and verification gate | require the artifact before claiming the reference is operationally usable |
...
Codex Plugin Creator Patterns stops being sufficient when the task spans multiple independent systems, more than one ownership boundary, unbounded source discovery, or production traffic without an explicit SLO.
...
| --- | --- |
| `official_docs_query_phrase` | codex plugin creator patterns official documentation best practices |
| `github_repository_query_phrase` | codex plugin creator patterns GitHub repository examples |
| `release_notes_query_phrase` | codex plugin creator patterns changelog release notes migration |
