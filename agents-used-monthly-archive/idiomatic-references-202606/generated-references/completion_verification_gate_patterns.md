...
Primary user starting state: The user has a `completion_verification_gate_patterns` task, one or more local source paths, and uncertainty about which pattern should drive implementation.
Decision being made: choosing the requirement IDs, negative cases, verification gates, and handoff evidence.
Reference opening trigger: Open this file when the task mentions completion verification gate patterns, any mapped local source path, or an adjacent workflow with the same failure mode.

## Decision Tradeoff Guide

| decision_option_name | when_to_choose_condition | tradeoff_cost_description | verification_question_prompt |
| --- | --- | --- | --- |
| Adopt when | local corpus and external evidence agree on the completion verification gate patterns pattern | fastest path, but can copy stale local assumptions | Does the selected pattern appear in the canonical source and current external evidence? |
| Adapt when | local sources are strong but public ecosystem guidance has changed | preserves repo fit, but requires explicit inference notes | Did the reference label the local fact, external fact, and combined inference separately? |
| Avoid when | source evidence is thin, conflicting, or unrelated to the user journey | prevents false confidence, but may require deeper research | Is there a confidence warning or adjacent reference route? |
| Cost of being wrong | wrong completion verification gate patterns guidance can send an agent to the wrong files, tests, or abstraction | wasted implementation loop and weaker verification | Would a reviewer know what to undo and what to inspect next? |
...
| --- | --- | --- | --- |
| agents-used-monthly-archive/claude-skills-202603/superpowers/verification-before-completion/SKILL.md | canonical primary source | Verification Before Completion; Overview; The Iron Law | What guidance, warning, or example should this source contribute to Completion Verification Gate Patterns? |
| claude-skills/superpowers/verification-before-completion/SKILL.md | supporting context source | Verification Before Completion; Overview; The Iron Law | What guidance, warning, or example should this source contribute to Completion Verification Gate Patterns? |

## Theme Specific Artifact

Theme specific artifact: worked completion verification gate patterns example with user goal, decision point, failure mode, and verification gate.

| artifact_field_name | artifact_completion_rule | evidence_source_hint |
| --- | --- | --- |
| user_goal_statement | state the user's concrete goal before applying Completion Verification Gate Patterns | local corpus hierarchy plus critique findings |
| decision_boundary_rule | define the point where this reference should be used or avoided | decision tradeoff guide |
| verification_gate_rule | define the command, checklist, or review question that proves the artifact worked | verification gate command set |

## Worked Example Set

Good example: Use Completion Verification Gate Patterns after loading the canonical source, confirming the external evidence boundary, and writing a verification gate before implementation.
Bad example: Use Completion Verification Gate Patterns as a generic tutorial while ignoring the mapped local paths, source hierarchy, and cost of being wrong.
Borderline case: Use Completion Verification Gate Patterns only after adding a confidence warning when local evidence is thin or conflicts with current ecosystem guidance.
...
- The role scenario names the user, starting state, decision, and trigger for Completion Verification Gate Patterns.
...
Adjacent reference 1: consider the completion adjacent reference when the current task pivots away from completion verification gate patterns.
Adjacent reference 2: consider the verification adjacent reference when the current task pivots away from completion verification gate patterns.
Adjacent reference 3: consider the gate adjacent reference when the current task pivots away from completion verification gate patterns.

## Workload Model

`combined_evidence_inference_note`: Treat Completion Verification Gate Patterns as a quality gate operating reference, not as a prose summary.
...
| source_pressure_model | local heading signals include Verification Before Completion; Overview; The Iron Law; The Gate Function; Common Failures; Red Flags - STOP; Verification Before Completion; Overview | compare guidance against canonical local sources before following external examples |
| artifact_pressure_model | required artifact is worked completion verification gate patterns example with user goal, decision point, failure mode, and verification gate | require the artifact before claiming the reference is operationally usable |
...
Completion Verification Gate Patterns stops being sufficient when the task spans multiple independent systems, more than one ownership boundary, unbounded source discovery, or production traffic without an explicit SLO.
