...
`combined_evidence_inference_note`: Treat Agent Creation Prompt Patterns as a agent workflow operating reference, not as a prose summary.
...
| source_pressure_model | local heading signals include Agent Development for Claude Code Plugins; Overview; Agent File Structure; Complete Format; Frontmatter Fields; name (required); AI-Assisted Agent Gen | compare guidance against canonical local sources before following external examples |
| artifact_pressure_model | required artifact is agent charter template with owner, user, trigger, tool budget, escalation, and retirement rule | require the artifact before claiming the reference is operationally usable |
...
Agent Creation Prompt Patterns stops being sufficient when the task spans multiple independent systems, more than one ownership boundary, unbounded source discovery, or production traffic without an explicit SLO.
