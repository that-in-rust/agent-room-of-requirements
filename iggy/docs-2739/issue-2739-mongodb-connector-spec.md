...
| Crate | Path | Change | Why |
|---|---|---|---|
| **mongodb_sink** | `core/connectors/sinks/mongodb_sink/` | Major rewrite of `connect()`, add config fields, fix error handling | Switches from `Client::with_uri_str()` to `ClientOptions::parse()` pattern |
| **mongodb_source** | `core/connectors/sources/mongodb_source/` | Major: add retry loop, wire 4 dead config fields, fix error types, rename tests | Source is currently non-production-ready without retry |
| **Root workspace** | `Cargo.toml`, `Cargo.lock` | Add `"mongodb"` feature to `testcontainers-modules` | E2E test infrastructure |
| **integration tests** | `core/integration/tests/connectors/` | New: fixtures + test files for MongoDB | ~8 new files |

### 9.3 SDK Interfaces Consumed (no changes to SDK itself)
...
| `Schema` enum | Source | `Json` (default), `Raw` (for BSON), `Text` (for string). Currently hardcoded to `Json`. |
| `sink_connector!()` / `source_connector!()` | Both | FFI registration macros. No change needed. |

### 9.4 External Libraries

| Library | Version | Used By | Role |
|---|---|---|---|
| `mongodb` (driver) | 3.x | Both | `ClientOptions`, `Client`, `Collection`, `Document`, `find()`, `insert_many()` |
| `bson` | (via mongodb) | Both | `doc!{}` macro, `Bson`, `Document`, `ObjectId` timestamp extraction |
| `tokio` | workspace | Both | Async runtime, `sleep()` for retry backoff |
| `serde` / `serde_json` | workspace | Both | Config deserialization, JSON payload serialization |
