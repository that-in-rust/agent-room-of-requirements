...
impl MongoDBSink {
    ...
    async fn process_messages(
        ...
        if self.verbose {
            info!(
                "MongoDB sink ID: {} processed {msg_count} messages to collection '{collection_name}'",
                self.id
            );
        } else {
            debug!(
        ...
        match payload_format {
            PayloadFormat::Document => {
                ...
                let json_value: serde_json::Value = serde_json::from_slice(&payload_bytes)
                    .map_err(|e| Error::InvalidJsonPayload)?;
            ...
            PayloadFormat::Text => {
                let text = String::from_utf8(payload_bytes)
                    .map_err(|e| Error::InvalidTextPayload)?;
    ...
    async fn execute_insert_many_with_retry(
        ...
        loop {
            match collection.insert_many(docs.clone()).with_options(options.clone()).await {
                Ok(result) => {
                    if self.verbose {
                        info!(
...
use mongodb::options::{
    ChangeStreamOptions, ClientOptions, FindOptions, FullDocumentType,
};
use serde::{Deserialize, Serialize};
...
impl MongoDBSource {
    ...
    async fn verify_change_stream_support(&self) -> Result<(), Error> {
        ...
        // isMaster / hello command returns "setName" field if part of replica set
        let result = db.run_command(doc! { "hello": 1 })
            .await
            .map_err(|e| Error::InitError(format!("Failed to run hello command: {e}")))?;

        if result.get("setName").is_none() && result.get("msg").is_none() {
            return Err(Error::InitError(
                "MongoDB change streams require a replica set or Atlas. \
    ...
    async fn poll_collections(&self) -> Result<Vec<ProducedMessage>, Error> {
        ...
        for collection_name in &self.config.collections {
            ...
            let mut cursor = with_retry_async(
                || collection.find(filter.clone()).with_options(options.clone()),
...
impl MongoDBSource {
    async fn poll_change_stream(&self) -> Result<Vec<ProducedMessage>, Error> {
        ...
        // Build pipeline to filter by collection if filter_collections is set
        let pipeline = self.build_change_stream_pipeline(&capture_ops);

        // Build options
        let mut cs_options = ChangeStreamOptions::builder()
            .batch_size(batch_size)
            .full_document(if full_doc {
            ...
            match serde_json::from_str::<serde_json::Value>(token_json) {
                Ok(json_val) => {
                    match mongodb::bson::to_document(&json_val) {
                        Ok(token_doc) => {
                            cs_options.resume_after = Some(token_doc);
                            info!(
        ...
        while messages.len() < batch_size as usize {
            // Use try_next with a very short timeout to avoid blocking indefinitely
            match tokio::time::timeout(
                ...
                Ok(Some(Err(e))) => {
                    ...
                    if is_transient_mongodb_error(&e) {
                        // Return what we have; runtime will call poll() again
                        break;
                    }
    ...
    loop {
        match operation().await {
            ...
            Err(e) => {
                ...
                tokio::time::sleep(delay * attempts).await;
            }
        }
    }
}
...
**Note on the `mongodb` crate version**: The mongodb crate v3.x (released 2024) is the current
stable API with async-first design. It depends on `tokio` which is already a workspace dependency.
The `tokio-runtime` feature enables the async client.
...
- On restart, it is deserialized back to a BSON Document and passed to
  `ChangeStreamOptions.resume_after`.
...
loop {
    match operation().await {
        ...
        Err(e) => {
            ...
            // Exponential backoff: delay * attempt number (1x, 2x, 3x)
            tokio::time::sleep(retry_delay * attempts).await;
        }
    }
}
...
Key points:
- `retry_delay` is parsed from a humantime string (e.g., `"1s"`, `"500ms"`) in the constructor.
- Backoff is linear multiplicative (`retry_delay * attempts`), exactly as PostgresSink uses it.
...
```rust
    ...
    fn given_arbitrary_string_should_stay_as_string_bson() {
        let bson = offset_string_to_bson("2024-01-01T00:00:00Z");
...
- [ ] Write `src/lib.rs`:
  ...
  - [ ] `process_messages()` — batch chunking, error counting (section 5.7)
  ...
  - [ ] `execute_insert_many_with_retry()` — retry loop (section 5.7)
...
- [ ] Write `src/lib.rs`:
  ...
  - [ ] `MongoChangeRecord` struct (section 6.8)
  - [ ] `MongoDBSource::new()` with state restoration (section 6.4)
  - [ ] `serialize_state()` helper (section 6.4)
  - [ ] `impl Source for MongoDBSource` — `open()`, `poll()`, `close()` (section 6.5)
  - [ ] `connect()` (section 6.6)
  - [ ] `get_database()` helper (section 6.6)
  - [ ] `verify_change_stream_support()` — hello command check (section 6.6)
  - [ ] `poll_collections()` — find with offset, state update (section 6.6)
  - [ ] `build_polling_filter()` — filter construction (section 6.6)
  - [ ] `mark_or_delete_processed_documents()` (section 6.6)
  - [ ] `poll_change_stream()` — watch, drain with timeout, token tracking (section 6.7)
  - [ ] `build_change_stream_pipeline()` — operationType and ns.coll filters (section 6.7)
  - [ ] `change_event_to_message()` — ChangeStreamEvent to ProducedMessage (section 6.7)
  - [ ] `bson_to_offset_string()` (section 6.9)
  - [ ] `offset_string_to_bson()` (section 6.9)
  - [ ] `bson_document_to_json()` (section 6.9)
  - [ ] `with_retry_async()` (section 6.10)
...
are opened directly with `db.watch()` using the `ChangeStreamOptions`. The only prerequisite check
